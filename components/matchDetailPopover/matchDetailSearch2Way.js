...
export default class MatchDetailSearch2Way extends React.Component {
  state = {
    isPopoverOpen: false,
    text: ""
  };
  ...
  clickCheck = (e, index) => {
    ...
    const filterdText = matchingDetail.filter(item => {
      return item.creditnoteItems.itemId.startsWith(this.state.text);
    });
    const changeItem = filterdText[index];
    const newMatchingDetail = matchingDetail.map(item => {
      if (changeItem.creditnoteItems.itemId === item.creditnoteItems.itemId) {
