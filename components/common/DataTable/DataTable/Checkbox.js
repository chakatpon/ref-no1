...
export default class Checkbox extends PureComponent {
  ...
  render() {
    ...
    const baseStyle =
      TagName !== "input" ? componentOptions.style : baseCheckboxStyle;
    const resolvedComponentOptions = handleFunctionProps(
      componentOptions,
      indeterminate
    );
    return TagName !== "input" ? (
      ...
      <div className="custom-control custom-checkbox">
        <input
          ...
          ref={this.checkbox}
          onClick={this.handleClick}
          name={data.externalId || name}
          aria-label={data.externalId || name}
          checked={checked}
          onChange={() => null}
          id={data.externalId || name}
        />
      </div>
    );
  }
