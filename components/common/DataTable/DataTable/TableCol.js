...
class TableCol extends PureComponent {
  ...
  render() {
    const { column, lang, t } = this.props;
            ...
            >
              {column.name && (
                <ColumnCellWrapper active={sortable}>
                  {sortable && sortIcon && (
                    <SortIcon className={sortDirection}>{sortIcon}</SortIcon>
                  )}
                  {t(`${lang}:${column.name.replace(/[.]/g, "")}`)}
                </ColumnCellWrapper>
              )}
            </TableColStyle>
          );
...
export default withTranslation(["request-edit"])(TableCol);
